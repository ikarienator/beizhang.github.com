...
function init1() {
    ...
    var json = { id : 'root', name : 'all', data : { $color : "#633" }, children : []};
    for (var i = 0; i < 3; i++) {
        var c = { id : 'c' + i, name : i, data : { $color : "#766" }, children : [] };
        json.children.push(c);
        var cs = Math.random() * 9 + 1, sumc = 0;;
        for (var j = 0; j < cs; j++) {
            var d = { id : 'c' + i + '_' + j, name : i + '/' + j, data : { $color : "#9e939f" }, children : [] };
            c.children.push(d);
            var ds = Math.random() * 9 + 1, sumd = 0;
            for (var k = 0; k < ds; k++) {
                var e = { id : 'c' + i + '_' + j + '_' + k, name : i + '/' + j + '/' + k, data : { $color : "#554" } };
